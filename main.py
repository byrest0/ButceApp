...
def main(page: ft.Page):
    ...
    def abonelikleri_kontrol_et():
        ...
        for ab in abonelikler:
            try:
                ...
                # HATA DÜZELTME: 'gun' değerini int'e çeviriyoruz
                if son_tarih.month != simdi.month and simdi.day >= int(ab['gun']):
                    islemler.append({
                        "baslik": f"{ab['baslik']} (Otomatik)",
                        "tutar": float(ab['tutar']), # Tutarı float yapalım
                        "tur": "gider",
    ...
    def verileri_yedekle(e):
        try:
            dosya_adi = "cepte_butce_yedek.csv"
            with open(dosya_adi, mode='w', newline='', encoding='utf-8-sig') as file:
                writer = csv.writer(file)
                writer.writerow(["TÜR", "DETAYLAR..."])
                for i in islemler: writer.writerow(["İşlem", i['baslik'], i['tutar'], i['tur'], i['tarih']])
                for v in varliklar: writer.writerow(["Varlık", v['ad'], v['miktar'], v.get('detay','')])
                for h in hedefler: writer.writerow(["Hedef", h['baslik'], h['hedef'], h['biriken']])
                for n in notlar: writer.writerow(["Not", n.get('baslik',''), n['icerik'], n['tarih']])
                for ab in abonelikler: writer.writerow(["Abonelik", ab['baslik'], ab['tutar'], ab['gun'], ab.get('son_eklenme','')])
            bildirim_goster(f"Yedeklendi: {dosya_adi}", "green")
    ...
    # 2. EKLEME SAYFASI (GÜNCELLENDİ: YÖNET BUTONU EKLENDİ)
    def add_view():
    ...
    # --- 5. NOTLAR SAYFASI (BAŞLIK EKLENDİ) ---
    def notes_view():
